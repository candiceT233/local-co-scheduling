    ...
    hid_t file_space_id, void * obj, hid_t dxpl_id, const void *buf)
{
    ...
    file_prov_info_t * file_info = (file_prov_info_t*)dset_info->obj_info.file_info;
    ...
    size_t size, void * blob_id, const void * buf,void *ctx)
{
    ...
    if(file_info->opened_datasets_cnt > 0){
        ...
        dataset_prov_info_t * dset_info = (dataset_prov_info_t*) &file_info->opened_datasets[0];
        assert(dset_info);
...
#endif
#ifdef DATA_PROVNC_LOGGING_MORE
    ...
    if(args->op_type == H5VL_OBJECT_REFRESH) {
        ...
        if(ret_value >= 0) {
            ...
            if(my_type == H5I_DATASET) {
                ...
                printf("H5VL_provenance_object_specific dset_id[%ld]",dataset_id); // not used!
                H5Sclose(space_id);
