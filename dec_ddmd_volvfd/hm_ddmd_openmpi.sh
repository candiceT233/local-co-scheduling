...
nost_ip=`getent hosts "$HOSTNAME_PREFIX.ibnet" | awk '{ print $1 }'`
echo "$nost_ip" >> ./host_ip
...
OPENMM () {
    ...
    if [ "$S1_HM" == true ]
    then
        LD_PRELOAD=${HERMES_INSTALL_DIR}/lib/hermes_vfd/libhdf5_hermes_vfd.so:$LD_PRELOAD \
    ...
    #PYTHONPATH=~/git/molecules/ python /people/leeh736/git/DeepDriveMD-pipeline/deepdrivemd/sim/openmm/run_openmm.py -c $yaml_path &>> $task_id.log &
    #LD_PRELOAD=~/git/tazer_forked/build.h5/src/client/libclient.so PYTHONPATH=~/git/molecules/ python /people/leeh736/git/DeepDriveMD-pipeline/deepdrivemd/sim/openmm/run_openmm.py -c /qfs/projects/oddite/leeh736/ddmd_runs/test/md_direct.yaml &> $task_id.log &
}
...
STOP_DAEMON() {
    ...
    mpirun --host $ib_hostlist --pernode \
        -x LD_PRELOAD=${HERMES_INSTALL_DIR}/lib/hermes_vfd/libhdf5_hermes_vfd.so \
...
# --exclude=dlt[02,10-12] --gres=gpu:8 --mem=0
